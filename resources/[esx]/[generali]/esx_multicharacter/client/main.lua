...
if ESX.GetConfig().Multichar then
	...
	SetupCharacter = function(index)
		...
		elseif Characters[index] and Characters[index].skin then
			...
			exports["fivem-appearance"]:setPedAppearance(PlayerPedId(), Characters[index].skin)
		end
		spawned = index
		local playerPed = PlayerPedId()
	...
	AddEventHandler('esx:playerLoaded', function(playerData, isNew, skin)
		...
		if isNew or not skin or #skin == 1 then
			...
			exports['fivem-appearance']:startPlayerCustomization(function(appearance)
				if (appearance) then
					TriggerServerEvent('saveskintodb', appearance)
					finished = true
				else
					print('Creazione del personaggio annullata')
				end
			end, configg)
		...
		-- if not isNew then TriggerEvent('skinchanger:loadSkin', skin or Characters[spawned].skin) end setPedAppearance
		-- if not isNew then exports["fivem-appearance"]:setPlayerAppearance(skin or Characters[spawned].skin) end
		if not isNew then exports["fivem-appearance"]:setPedAppearance(PlayerPedId(), skin or Characters[spawned].skin) end
		Citizen.Wait(400)
		DoScreenFadeIn(400)
		repeat Citizen.Wait(200) until not IsScreenFadedOut()
